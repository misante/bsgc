...
import { FaEdit, FaTrash, FaPlus, FaUserClock } from "react-icons/fa";
...
export default function ManpowerPlanningPage() {
  ...
  const fetchData = async () => {
    try {
      const [reqRes, projectsRes, manpowerRes] = await Promise.all([
        fetch("/api/planning/manpower"),
        fetch("/api/projects"),
        fetch("/api/master/manpower"),
      ]);

      const [reqData, projectsData, manpowerData] = await Promise.all([
        reqRes.json(),
        projectsRes.json(),
        manpowerRes.json(),
      ]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = "/api/planning/manpower";
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400">
              Plan and track manpower requirements
            </p>
          </div>
          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                const res = await fetch(
                                  `/api/planning/manpower?id=${req.id}`,
                                  {
                                    method: "DELETE",
                                  }
                                );
