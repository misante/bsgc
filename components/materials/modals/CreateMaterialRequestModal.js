...
// import { X, Package, User, FileText, Search, Building } from "lucide-react";
...
//   // Get project display name for dropdown options
//   const getSelectedProjectName = (project) => {
//     const name =
//       project?.name ||
...
//     const requestData = {
//       ...formData,
//       requested_by: currentUser.first_name,
//       project_name: getSelectedProjectName(),
//     };
//     console.log("project_name:", requestData.project_name);
//     try {
...
//         onClose();
//       } else {
//         toast(`Failed to create request: ${result.error}`);
//       }
//     } catch (error) {
...
//                         }))
//                       }
//                       className="w-full capitalize pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white appearance-none"
//                     >
...
//                       Selected: {getSelectedProjectName()}
//                     </div>
//                   )}
//                 </div>
...
const CreateMaterialRequestModal = ({
  ...
  // Fixed getSelectedProjectName function
  const getSelectedProjectName = () => {
    if (!formData.project_id) return "";

    console.log("🔍 Looking for project with ID:", formData.project_id);
    console.log("📋 Available projects:", projects);

    const selected = projects?.find(
      (project) =>
        project.id === formData.project_id ||
        project.id?.toString() === formData.project_id?.toString()
    );

    console.log("✅ Found project:", selected);

    // Try different possible property names for project name
    const projectName =
      selected?.name ||
  ...
  const handleSubmit = async (e) => {
    ...
    const requestData = {
      ...formData,
      requested_by: currentUser.first_name,
      project_name: getSelectedProjectName(), // Include project name for reference
    };
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(`Failed to create request: ${result.error}`);
      }
    } catch (error) {
      console.error("Error creating material request:", error);
            ...
            <div className="flex-1 overflow-y-auto">
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div>
                  ...
                  {formData.project_id && (
                    <div className="mt-2 p-2 capitalize bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded text-sm text-green-700 dark:text-green-300">
                      Selected: {getSelectedProjectName()}
                    </div>
                  )}
                </div>
